...
class OrganizerAIService {
  ...
  // Only fires when category IS a valid ObjectId.
  // ============================================================
  async _callPlanningAgent(payload) {
    const response = await api.post("/ai/organizer/plan-event", payload);
      ...
      try {
        ...
        if (suggestions?.price) {
          return {
            ...
            confidence:
              suggestions.price.confidence ||
              response?.data?.data?.plan?.confidence ||
              75,
            source: "ai",
          };
        }
  ...
  getMockPriceSuggestion(eventData) {
    ...
    const multiplier =
      multipliers[eventData?.category?.toLowerCase()] || multipliers.default;
  ...
  async getSlotSuggestion(location, category) {
    if (isObjectId(category)) {
      try {
        const suggestions = await this._callPlanningAgent({
          event_name: `${category || "Event"} in ${location || "Location"}`,
          category,
      ...
      try {
        ...
        if (suggestions?.dateTime) {
          return {
            ...
            bestDayOfWeek:
              suggestions.dateTime.suggestedDayOfWeek || "Saturday",
  ...
  getMockDateSuggestion(category) {
    ...
    return {
      ...
      bestDayOfWeek: ["Friday", "Saturday", "Sunday"][
  ...
  async createNegotiationOffer(requestId, offerData) {
    if (IS_DEV)
      console.info(
        "ℹ️ createNegotiationOffer: no backend route yet, using mock"
      );
    return {
      success: true,
  ...
  async getCompetitorAnalysis(requestId) {
    if (IS_DEV)
      console.info(
        "ℹ️ getCompetitorAnalysis: no backend route yet, using mock"
      );
  ...
  getMockCompetitorAnalysis() {
    ...
    return {
      analysis: {
        ...
        recommendation:
          "Focus on your experience and quality to justify premium pricing",
      },
      competitors,
    };
  }
  ...
  getMockDashboardMetrics(orgId, timeframe) {
    ...
    return {
      ...
      totalReviews: events.reduce(
        (s, e) => s + Math.floor(e.attendees * 0.3),
        0
      ),
  ...
  _transformSentimentData(d) {
    ...
    if (positivePct > 70)
      insights.push(
        `${positivePct}% of attendees left positive feedback — great work!`
      );
    if (negativePct > 25)
      insights.push(
        `${negativePct}% of reviews are negative — consider reviewing common issues.`
      );
    ...
    if (d.totalAnalyzed > 0)
      insights.push(
        `${d.totalAnalyzed} reviews analysed by AI sentiment engine.`
      );
  ...
  // getEventPerformance — restored (was dropped in previous revision)
  // Uses real backend: /ai/dashboard/revenue + /ai/dashboard/events
  async getEventPerformance(orgId, eventId = null) {
    try {
      ...
      if (IS_DEV)
        console.warn(
          "⚠️ getEventPerformance falling back to mock:",
          error.message
        );
      return { events: [] };
    }
  ...
  async checkPlanningAgentHealth() {
    const response = await api.get("/ai/organizer/planning-agent/health");
    return response.data ?? response;
  }

  async getPlanningAgentStats(timeRange = "30d") {
    const response = await api.get(
      `/ai/organizer/planning-agent/stats?timeRange=${timeRange}`
    );
