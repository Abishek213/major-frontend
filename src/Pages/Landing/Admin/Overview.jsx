...
const OverviewDashboard = () => {
    ...
    <div className="grid grid-cols-1 gap-8 mb-10 md:grid-cols-2 lg:grid-cols-4">
      {dashboardData.statsData.map(({ title, value, change, icon, color }, index) => {
        ...
        return (
          <div 
            key={title} 
            className={`bg-white border border-gray-200 rounded-2xl p-8 transition-all duration-500 hover:scale-[1.02] hover:shadow-xl group relative overflow-hidden`}
  ...
  const renderAnalyticsChart = () => (
    <div className={`bg-white border border-gray-200 rounded-2xl mb-10 shadow-lg`}>
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={`p-3 rounded-xl transition-all duration-200 border bg-gray-100/70 hover:bg-gray-200/70 border-gray-200/60 hover:scale-105`}>
        ...
        <div className={`h-96 p-4 rounded-xl bg-gray-50/30`}>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {Object.entries(dashboardData.eventStats).map(([status, count], index) => (
            <div 
              key={status} 
              className={`p-6 rounded-2xl transition-all duration-300 hover:scale-105 border bg-white border-gray-200 shadow-sm hover:shadow-md`}
              style={{ animationDelay: `${index * 50}ms` }}
            >
              <p className={`text-sm font-medium mb-3 capitalize text-gray-600`}>
                {status}
              </p>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {Object.entries(dashboardData.roleStats.distribution || {}).map(([role, data], index) => (
            <div 
              key={role} 
              className={`p-6 rounded-2xl transition-all duration-300 hover:scale-105 border group bg-white border-gray-200 hover:border-blue-300/50 shadow-sm hover:shadow-lg`}
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`p-2 rounded-xl group-hover:scale-110 transition-transform duration-300 bg-blue-100`}>
  ...
  const renderPermissionsAlert = () => {
    ...
    return (
      <Alert className={`mb-10 border-l-4 border-l-blue-500 rounded-2xl bg-blue-50/50 border-blue-200/50 shadow-sm`}>
