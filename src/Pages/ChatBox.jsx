...
const ChatBox = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 w-80 rounded-lg border overflow-hidden bg-white border-gray-300 shadow-lg">
      <div className="p-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-blue-800">Chat Support</h2>
          {/* Chatbot Icon */}
        ...
        <form onSubmit={handleSendMessage} className="flex">
          <input
            ...
            className="flex-grow border border-gray-300 bg-white text-black rounded-l-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
        ...
        <div className="mt-4">
          <h3 className="text-sm font-semibold text-black">Quick Questions:</h3>
          <div className="grid grid-cols-2 gap-2 mt-2">
            {presetQuestions.map((question, index) => (
              <button
                key={index}
                onClick={() => handlePresetQuestion(question)}
                className="bg-gray-200 hover:bg-gray-300 text-black px-2 py-1 rounded-md transition"
              >
                {question}
              </button>
            ))}
